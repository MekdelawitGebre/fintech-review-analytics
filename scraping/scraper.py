...
def scrape_app(pkg_name: str, bank_name: str, target: int = 600, lang="en", country="us"):
    ...
    try:
        ...
        logger.exception(
            "Failed to fetch all reviews for %s: %s", bank_name, e
        )
        all_reviews = []
...
def main(target_each: int = 600, out_csv: str = None):
    """Scrape all apps and save to CSV."""
    rows = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Scrape Google Play reviews for target apps."
    )
    parser.add_argument(
        "--target",
        type=int,
        default=600,
        help="Target reviews per app (default 600)",
    )
    parser.add_argument(
        "--out",
        type=str,
